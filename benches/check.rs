...
pub fn compare_benchmark(c: &mut Criterion) {
    ...
    let current_dir = env::current_dir().expect("get current dir");
    let app = dotenv_linter::cli::new(current_dir.as_os_str());
    let matches = app.get_matches_from(vec!["dotenv-linter", "compare", ".env", ".env.compare"]);
    ...
    c.bench_function("dotenv_linter compare", |b| {
        ...
        b.iter(|| dotenv_linter::compare(black_box(&matches), black_box(&path)))
    });
}

criterion_group!(benches, check_benchmark, compare_benchmark);
